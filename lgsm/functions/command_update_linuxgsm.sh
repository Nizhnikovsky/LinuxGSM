...
curl --connect-timeout 10 -IsfL "https://${gittoken}@raw.githubusercontent.com/${githubuser}/${githubrepo}/${githubbranch}/linuxgsm.sh" 1>/dev/null
if [ $? != "0" ]; then
	curl --connect-timeout 10 -IsfL "https://${gittoken}@github.com/${githubuser}/${githubrepo}/raw/${githubbranch}/linuxgsm.sh" 1>/dev/null
	if [ $? != "0" ]; then
...
if [ "${remotereponame}" == "GitHub" ]; then
	tmp_script_diff=$(diff "${tmpdir}/linuxgsm.sh" <(curl --connect-timeout 10 -s "https://${gittoken}@raw.githubusercontent.com/${githubuser}/${githubrepo}/${githubbranch}/linuxgsm.sh"))
else
	tmp_script_diff=$(diff "${tmpdir}/linuxgsm.sh" <(curl --connect-timeout 10 -s "https://${gittoken}@github.com/${githubuser}/${githubrepo}/raw/${githubbranch}/linuxgsm.sh"))
fi
...
if [ "${remotereponame}" == "GitHub" ]; then
	curl --connect-timeout 10 -IsfL "https://${gittoken}@raw.githubusercontent.com/${githubuser}/${githubrepo}/${githubbranch}/lgsm/config-default/config-lgsm/${gameservername}/_default.cfg" 1>/dev/null
else
	curl --connect-timeout 10 -IsfL "https://${gittoken}@github.com/${githubuser}/${githubrepo}/raw/${githubbranch}/lgsm/config-default/config-lgsm/${gameservername}/_default.cfg" 1>/dev/null
fi
if [ $? != "0" ]; then
	fn_print_fail_eol_nl
...
if [ "${remotereponame}" == "GitHub" ]; then
	config_file_diff=$(diff "${configdirdefault}/config-lgsm/${gameservername}/_default.cfg" <(curl --connect-timeout 10 -s "https://${gittoken}@raw.githubusercontent.com/${githubuser}/${githubrepo}/${githubbranch}/lgsm/config-default/config-lgsm/${gameservername}/_default.cfg"))
else
	config_file_diff=$(diff "${configdirdefault}/config-lgsm/${gameservername}/_default.cfg" <(curl --connect-timeout 10 -s "https://${gittoken}@github.com/${githubuser}/${githubrepo}/raw/${githubbranch}/lgsm/config-default/config-lgsm/${gameservername}/_default.cfg"))
fi
...
if [ -n "${functionsdir}" ]; then
	if [ -d "${functionsdir}" ]; then
		...
		for functionfile in *; do
			...
			if [ "${remotereponame}" == "GitHub" ]; then
				curl --connect-timeout 10 -IsfL "https://${gittoken}@raw.githubusercontent.com/${githubuser}/${githubrepo}/${githubbranch}/${github_file_url_dir}/${functionfile}" 1>/dev/null
			else
				curl --connect-timeout 10 -IsfL "https://${gittoken}@github.com/${githubuser}/${githubrepo}/raw/${githubbranch}/${github_file_url_dir}/${functionfile}" 1>/dev/null
			fi
			if [ $? != 0 ]; then
				...
				# compare file
				if [ "${remotereponame}" == "GitHub" ]; then
					function_file_diff=$(diff "${functionsdir}/${functionfile}" <(curl --connect-timeout 10 -s "https://${gittoken}@raw.githubusercontent.com/${githubuser}/${githubrepo}/${githubbranch}/${github_file_url_dir}/${functionfile}"))
				else
					function_file_diff=$(diff "${functionsdir}/${functionfile}" <(curl --connect-timeout 10 -s "https://${gittoken}@github.com/${githubuser}/${githubrepo}/raw/${githubbranch}/${github_file_url_dir}/${functionfile}"))
				fi
